...
def compute_bosdis(messages: np.ndarray, observations: np.ndarray) -> float:
    ...
    messages: Union[list, np.ndarray]
        Messages to calculate the compute_bosdis for.
    observations: Union[list, np.ndarray]
        Meanings to calculate the compute_bosdis for.
    ...
    bow_message = []
    bow_meaning = []
    for meaning, message in zip(observations, messages):
    ...
    return compute_posdis(messages=bow_message, observations=bow_meaning)
